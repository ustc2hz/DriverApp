 ...
 * 
 * // 设置合成 mTts.setParameter(SpeechConstant.ENGINE_TYPE,
 * SpeechConstant.TYPE_CLOUD); // 设置播放器音频流类型
 * mTts.setParameter(SpeechConstant.STREAM_TYPE, "" +
 * mContext.getString(R.string.preference_key_tts_stream));
 * 
 * // 设置发音人 mTts.setParameter(SpeechConstant.VOICE_NAME,
 * mContext.getString(R.string.preference_default_tts_role)); // 设置语速
 * mTts.setParameter(SpeechConstant.SPEED, "" +
 * mContext.getString(R.string.preference_key_tts_speed)); // 设置音量
 * mTts.setParameter(SpeechConstant.VOLUME, "" +
 * mContext.getString(R.string.preference_key_tts_volume)); // 设置语调
 * mTts.setParameter(SpeechConstant.PITCH, "" +
 * mContext.getString(R.string.preference_key_tts_pitch));
 * 
 * }
 * 
 * public void startSpeaking() { // 初始化合成对象 mTts =
 * SpeechSynthesizer.createSynthesizer(mContext, mTtsInitListener); isfinish =
 * true; setParam();
 * 
 * int code = mTts.startSpeaking(mContext, mTtsListener); if (code !=
 * ErrorCode.SUCCESS) { if(code == ErrorCode.ERROR_COMPONENT_NOT_INSTALLED){
 * //未安装则跳转到提示安装页面 mInstaller.install(); }else { showTip("语音合成失败,错误码: " + code);
 * } }
 * 
 * }
 * 
 * public void stopSpeaking() { if (mTts != null) { mTts.stopSpeaking(); } }
 * 
 * }
 */