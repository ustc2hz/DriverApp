...
import ustc.sse.water.data.model.*;

public class BActivity extends Activity implements OnClickListener{
	//button的申明
	private Button changeMess,commit,back ;
	//edittext的申明
	private EditText park_number,l_price,m_price,h_price,pl_price,pm_price,ph_price;
	//SharedPreference获取当前的发布信息
	SharedPreferences preferences ;
	SharedPreferences.Editor editor;
	//用于获取edittext的值
	private String num,price_ten,price_twenty,price_thirty,pprice_ten,pprice_twenty,pprice_thirty;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		private void initData() {
			...
			if (park_number.getText() != null) {
				dData.setOrderSpaceSum(park_number.getText().toString().trim());
			}
			if (l_price.getText() != null) {
				dData.setOrderOfTen(l_price.getText().toString().trim());
			}
			if (m_price.getText() != null) {
				dData.setOrderOfTwe(m_price.getText().toString().trim());
			}
			if (h_price.getText() != null) {
				dData.setOrderOfTri(h_price.getText().toString().trim());
			}
			if (pl_price.getText() != null) {
				dData.setSpaceOfHal(pl_price.getText().toString().trim());
			}
			if (pm_price.getText() != null) {
				dData.setSpaceOfOne(pm_price.getText().toString().trim());
			}
			if (ph_price.getText() != null) {
	...
	public void onClick(View v) {
		...
		case R.id.bt1:
			...
			case R.id.bt3:
				// 点击返回，返回主界面
				Intent intent1 = new Intent(BActivity.this,
						MainActivity.class);
	...
	public void SaveData(){
		...
		m_price.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				// TODO Auto-generated method stub
				price_twenty = preferences.getString("price_twenty", price_twenty);	
		...
		h_price.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				// TODO Auto-generated method stub
				price_thirty = preferences.getString("price_thirty", price_thirty);
		...
		pm_price.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				pprice_twenty = preferences.getString("pprice_twenty", pprice_twenty);
		...
		ph_price.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				pprice_thirty = preferences.getString("pprice_thirty", pprice_thirty);
